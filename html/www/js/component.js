(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      ((global.AlpineMagicHelpers = global.AlpineMagicHelpers || {}),
      (global.AlpineMagicHelpers.component = factory())));
})(this, function () {
  "use strict";

  const checkForAlpine = function checkForAlpine() {
    if (!window.Alpine) {
      throw new Error(
    ...
    if (
      ...
      !isValidVersion("2.5.0", window.Alpine.version)
    ) {
      throw new Error(
        "Invalid Alpine version. Please use Alpine version 2.5.0 or above"
      );
    }
  };
  const syncWithObservedComponent = function syncWithObservedComponent(
    data,
    observedComponent,
    callback
  ) {
    ...
    const handler = function handler(scope) {
      if (scope === void 0) {
        scope = null;
      }
      ...
      return {
        get: function get(target, key) {
          ...
          if (
            ...
            (_observedComponent$__ = observedComponent.__x) != null &&
            _observedComponent$__.$data[key]
          ) {
  ...
  const updateOnMutation = function updateOnMutation(
    componentBeingObserved,
    callback
  ) {
    if (!componentBeingObserved.getAttribute("x-bind:data-last-refresh")) {
      componentBeingObserved.setAttribute(
        "x-bind:data-last-refresh",
        "Date.now()"
      );
    }
    ...
    const observer = new MutationObserver(function (mutations) {
      for (let i = 0; i < mutations.length; i++) {
        ...
        if (
          mutatedComponent &&
          !mutatedComponent.isSameNode(componentBeingObserved)
        )
          continue;
        callback();
        return;
  ...
  const objectSetDeep = function objectSetDeep(object, path, value) {
  ...
  const componentData = function componentData(component, properties) {
    const data = component.__x
      ? component.__x.getUnobservedData()
      : saferEval(component.getAttribute("x-data"), component);
  ...
  function isValidVersion(required, current) {
    ...
    for (let i = 0; i < requiredArray.length; i++) {
      if (
        !currentArray[i] ||
        parseInt(currentArray[i]) < parseInt(requiredArray[i])
      ) {
        return false;
      }
    }
  ...
  function saferEval(expression, dataContext, additionalHelperVariables) {
    ...
    return new Function(
      ...
      void 0,
      [dataContext].concat(Object.values(additionalHelperVariables))
    );
    ...
    start: function start() {
      ...
      Alpine.addMagicProperty("component", function ($el) {
        return function (componentName) {
          ...
          const componentBeingObserved = document.querySelector(
